...
ignore.zeros = T
n.random.iterations = 10
unique.stamp = str_c(select.gender, "_", "Nonzerofraction", non.zero.fraction, if (ignore.zeros) "_IgnoreZeros" else "", "_", n.random.iterations, "RandomIterations_", str_replace_all(format(Sys.time(), "%X"), ":", ""))
...
# Determine FDR
print("============== Determine FDR =================")
p.values.random = NULL
for (i in 1:n.random.iterations)
{
	print(str_c("~-~-~-~-~- Iteration ", i, " of ", n.random.iterations, " ~-~-~-~-~-"))
	...
	# Get p-values
	p.values.random = c(p.values.random, as.vector(random$cor.matrix.p))
}
...
## Which have FDR < 0.05?
#
fdr = fdr.manyIterations(p.values.real, p.values.random, n.random.iterations, n.elements = 2e3)

# Select p-values for which FDR < .05
index.fdr = max(which(fdr$fdr <= .05))
hits = is.finite(index.fdr)
p.values.significant = if (!hits) NULL else fdr$p[1:index.fdr] #which(fdr$fdr <= .05)]

if (T) {
	pdf(str_c('plots/', 'fdr_', unique.stamp, '.pdf'))
		n.elements.zoom = min(1e2, length(fdr$p))
		plot(log(fdr$p[1:n.elements.zoom], 10), fdr$fdr[1:n.elements.zoom], t='b', pch=19, lwd=2, axes=F, xlab="log( p-value )", ylab="FDR", col="gray", main = str_c("#signif = ", if (hits) index.fdr else 0))
		axis(1); axis(2, las=2)
		...
		{
			par(new = T)
			plot(log(fdr$p[1:n.elements.zoom], 10), fdr$fdr[1:n.elements.zoom], t = 'p', pch=c(rep(1, index.fdr), rep(NA, n.elements.zoom - index.fdr)), col="red", axes=F, xlab="", ylab="", lwd = 2)	
		}
	dev.off()
}
...
# Also show p-value distribution:
if (F)
{
	pdf('plots/random_p-value_distribution_two-part.pdf')
		hist(p.values.random, axes = F, xlab = "p-values", main = "Histogram of 'random' p-values two-part model")
		axis(1)
		axis(2, las = 2)
	dev.off()
	pdf('plots/random_p-value_distribution_spearman.pdf')
		hist(as.vector(random$cor.matrix.p), axes = F, xlab = "p-values", main = "Histogram of 'random' p-values, spearman")
		axis(1)
		axis(2, las = 2)
	dev.off()
}

# Save image with R-data enabling a reuse of 'random iterations'
save.image(str_c("/Volumes/lldeep/rimages/", unique.stamp, ".RData"))
